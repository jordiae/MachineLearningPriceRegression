...
###creem nova variable (ocean proximity)
...
dfOK <- df
# prova indexosln(MEDIAN AGE)	       0.1570  33.6123
df$oceanProximity <- log(df$oceanProximity)
df$roomsPop <- log(df$totalRooms/df$population)
df$bedsPop <- log(df$totalBedrooms/df$population)
df$popHouses <- log(df$population/df$households)
...
df$Col <- rbPal(1000)[as.numeric(cut(df$medianHouseValue,breaks = 1000))]
#df$Col <- rbPal(100)[as.numeric(log(df$medianHouseValue))]
plot(df$longitude,df$latitude,col=df$Col)
...
#This adds a column of color values
# based on the y values
df$Col <- rbPal(1000)[as.numeric(cut(df$oceanProximity,breaks = 1000))]
#df$Col <- rbPal(100)[as.numeric(log(df$medianHouseValue))]
plot(df$longitude,df$latitude,col=df$Col)
df$Col <- NULL
...
dfSenseLabel$longitude <- NULL
dfSenseLabel$latitude <- NULL
myPCA <- prcomp(dfSenseLabel,scale=TRUE)
# > myPCA$sdev
...
barplot(100*cumsum(pc1$sdev[1:dim(dfSenseLabel)[2]]^2)/dim(dfSenseLabel)[2])
percInerAccum<-100*cumsum(pc1$sdev[1:dim(dfSenseLabel)[2]]^2)/dim(dfSenseLabel)[2]
percInerAccum
...
#df2 <-df
#df2$x = cos(df2$latitude) * cos(df2$longitude)
#df2$y = cos(df2$latitude) * sin(df2$longitude)
#df2$z = sin(df2$latitude) 
#df <- df2
### RESAMPLING
...
varnames2 <-c(varnames,"oceanProximity")

# Noteu que glm sempre afegeix un terme "intercept" ó "offset" (un regressor constant 1) per defecte, així que tenim dues opcions:
#learn.data$medianIncome <- log(learn.data$medianIncome)
#learn.data$housingMedianAge <- log(learn.data$housingMedianAge)
#learn.data$roomsPerPop <- log(learn.data$totalRooms/learn.data$population)
#learn.data$bedsPerPop <- log(learn.data$totalBedrooms/learn.data$population)
#learn.data$popPerHouseholds <- log(learn.data$population/learn.data$households)
#learn.data$households <- log(learn.data$households)

#model.linear <- glm (medianHouseValue ~ ., data=learn.data[, !names(learn.data) %in% c("latitude", "longitude")] , family = gaussian)
learn.data2 <- learn.data
...
prediction <- predict(model.linear,newdata = learn.data)#[,!names(learn.data) %in% c("latitude", "longitude")])
N <- nrow(learn.data)
...
bestSVM <- tune(svm, medianHouseValue~., data = learn.data, kernel="radial",
                ...
                tunecontrol = tune.control(sampling = "cross",nrepeat=10), scale = TRUE
)
...
varnames3 <- c(varnames,"oceanProximity","roomsPop","popHouses","households")
library(MASS)
library(nnet)
set.seed (4567)
learn.data.scaled <- scale(learn.data)
learn.data.scaled <- as.data.frame(scale(learn.data), col.names = c(varnames3))
learn.data.scaled$medianHouseValue <- learn.data$medianHouseValue - mean(learn.data$medianHouseValue)
model.nnet <- nnet(medianHouseValue ~., data = learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))], size=30, maxit=1000, decay=0, linout = TRUE)

model.nnet 

prediction <- predict(model.nnet,newdata = learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
N <- nrow(learn.data.scaled)
#(mean.square.error <- sum((learn.data$medianHouseValue - prediction)^2)/N)
rmse <- sqrt(mean((learn.data.scaled$medianHouseValue - prediction)^2))
rmse



#df2 <- dfOK
#df2$oceanProximity <- df$oceanProximity

#library(randomForest)
#model.rf <- randomForest(medianHouseValue ~ ., data = dfOK, proximity=FALSE)
#prediction <- predict(model.rf,newdata = dfOK)
#N <- nrow(dfOK)
#(mean.square.error <- sum((learn.data$medianHouseValue - prediction)^2)/N)
#rmse <- sqrt(mean((dfOK$medianHouseValue - prediction)^2))
#rmse

library(randomForest)
model.rf <- randomForest(medianHouseValue ~ ., data = learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))], proximity=FALSE)
prediction <- predict(model.rf,newdata = learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
N <- nrow(learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
(mean.square.error <- sum((learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))]$medianHouseValue - prediction)^2)/N)
rmse <- sqrt(mean((learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))]$medianHouseValue - prediction)^2))
rmse

test.data.scaled <- scale(test.data)
test.data.scaled <- as.data.frame(scale(test.data), col.names = c(varnames3))
test.data.scaled$medianHouseValue <- test.data$medianHouseValue - mean(test.data$medianHouseValue)

prediction <- predict(model.rf,newdata = test.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
N <- nrow(test.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
(mean.square.error <- sum((test.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))]$medianHouseValue - prediction)^2)/N)
rmse <- sqrt(mean((test.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))]$medianHouseValue - prediction)^2))
rmse
...
#model3 <- svm (learn.data.scaled[,!(colnames(df) %in% c("medianHouseValue", "population", "totalRooms","totalBedrooms"))],learn.data.scaled$medianHouseValue,epsilon=0.01,gamma=10)
model3 <- svm (medianHouseValue ~ ., data = learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))],epsilon=0.01,gamma=10)
prediction <- predict(model3,newdata = learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
N <- nrow(learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))])
(mean.square.error <- sum((learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))]$medianHouseValue - prediction)^2)/N)
rmse <- sqrt(mean((learn.data.scaled[,!(colnames(df) %in% c("population", "totalRooms","totalBedrooms"))]$medianHouseValue - prediction)^2))
rmse
